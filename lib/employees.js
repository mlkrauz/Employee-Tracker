...
export const allEmployees = async () => {
    return await db.promise().query(`SELECT employee.id, employee.first_name, employee.last_name, role.title, department.name, role.salary, 
...
export const allManagers = async () => {
    ...
    LEFT OUTER JOIN employee manager
    ON employee.manager_id = manager.id
    INNER JOIN role ON employee.role_id = role.id AND role.isManager = 1
    INNER JOIN department ON role.department_id = department.id)`)
}
...
export const employeesByDepartment = async (deptId) => {
    ...
    return await db.promise().query(
        `SELECT employee.id, employee.first_name, employee.last_name, role.title, department.name, role.salary, 
        (SELECT CONCAT(manager.first_name, ' ', manager.last_name)) AS Manager
        FROM employee WHERE role_id IN (SELECT role_id FROM role WHERE department_id = (?))
        LEFT OUTER JOIN employee manager
        ON employee.manager_id = manager.id
        INNER JOIN role ON employee.role_id = role.id
        INNER JOIN department ON role.department_id = department.id`,
